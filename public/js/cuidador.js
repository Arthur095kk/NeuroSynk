...
// public/js/cuidador.js - PARTE CORRIGIDA DO SALVAR TAREFA
...
window.salvarNovaTarefa = async function(event, pacienteId) {
    ...
    // Adicionar cuidadorId se disponível
    const cuidadorId = localStorage.getItem('userId') || localStorage.getItem('usuarioId');
    if (cuidadorId) {
        formData.append('idCuidador', cuidadorId);
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            let mensagem = dados.mensagem || dados.error || 'Erro ao criar tarefa.';
            if (dados.errors) {
                mensagem = dados.errors.map(e => `${e.path}: ${e.message}`).join('\n');
            }
            alert('❌ Erro: ' + mensagem);
        }
    } catch (error) {
